...
namespace OAnQuan
{
    public partial class frmOAnQuan3 : Form
    {
                    ...
                    if (directionForm.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        else
                        {
                            ...
                            if (!isPlayer1Turn)
                            {
                                ...
                                Greedy(tempBoard, AIScore, ref bestMove, ref moveRight);
                                if (bestMove != -1)
                                {
                                    if (moveRight)
                                    {
        ...
        private void  Greedy(int[] board,int AIScore,ref int bestMove,ref bool moveRight)
        {
            int maxScore = 0;
            for (int i = 6; i < 11; i++)
            {
                if (board[i] > 0)
                {
                    ...
                    int[] tempBoard = (int[])board.Clone();
                    SimulateRightMove(tempBoard, i, ref tempAIScore);
                    if(tempAIScore>maxScore)
                    {
                        maxScore = tempAIScore;
                        bestMove = i;
                        moveRight = true;
                    }
                }
                if (board[i] > 0)
                {
                    int tempAIScore = AIScore;
                    int[] tempBoard = (int[])board.Clone();
                    SimulateLeftMove(tempBoard, i, ref tempAIScore);
                    if (tempAIScore > maxScore)
                    {
                        maxScore = tempAIScore;
                        bestMove = i;
        ...
        private void SimulateRightMove(int[] tempBoard, int startIndex, ref int AIScore)
        {
            ...
            {
                SimulateRightMove(tempBoard, (currentIndex + 1) % 12, ref AIScore);
            }
            else
            {
                CalculateScore3(tempBoard, currentIndex, ref AIScore);
            }
        }

        private void SimulateLeftMove(int[] tempBoard, int startIndex, ref int AIScore)
        {
            int count = tempBoard[startIndex];
            tempBoard[startIndex] = 0;
            int currentIndex = startIndex;
            ...
            {
                SimulateLeftMove(tempBoard, (currentIndex - 1 + 12) % 12, ref AIScore);
            }
            else
            {
                CalculateScore4(tempBoard, currentIndex, ref AIScore);
            }
        }

        private void CalculateScore3(int[] tempBoard, int currentIndex, ref int AIScore)
        {
            ...
            if (tempBoard[nextIndex] == 0 && tempBoard[tempIndex] != 0)
            {
                ...
                CalculateScore3(tempBoard, tempIndex, ref AIScore);
            }
        }
        private void CalculateScore4(int[] tempBoard, int currentIndex, ref int AIScore)
        {
            ...
            if (tempBoard[nextIndex] == 0 && tempBoard[tempIndex] != 0)
            {
                AIScore += tempBoard[tempIndex];
                tempBoard[tempIndex] = 0;
                CalculateScore4(tempBoard, tempIndex, ref AIScore);
            }
        }
