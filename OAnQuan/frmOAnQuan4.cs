...
namespace OAnQuan
{
    public partial class frmOAnQuan4 : Form
    {
                            ...
                            if (!isPlayer1Turn)
                            {
                                ...
                                for (int i = 6; i < 11; i++)
                                {
                                    if (board[i] > 0)
                                    {
                                        ...
                                        int moveValue1 = MinimaxAB(newBoard1, 10, false, ref AIScore, ref playerScore,-1000,1000);
                                        ...
                                        int moveValue2 = MinimaxAB(newBoard2, 10, false, ref AIScore, ref playerScore,-1000,1000);
        ...
        //Hàm MinimaxAB để lựa chọn nước đi 
        private int MinimaxAB(int[] board, int depth, bool isMaximizingPlayer, ref int AIScore, ref int playerScore, int alpha, int beta)
        {
            ...
            if (isMaximizingPlayer)
            {
                ...
                // Xét tất cả các ô của máy tính (ô từ 6 đến 10)
                for (int i = 6; i < 11; i++)
                {
                    if (board[i] > 0)
                    {
                        // Mô phỏng nước đi sang phải
                        int newAIScore = AIScore;
                        int newPlayerScore = playerScore;
                        int[] newBoard1 = (int[])board.Clone();
                        SimulateRightMove(newBoard1, i, ref newAIScore, ref newPlayerScore, isAI);
                        int score1 = MinimaxAB(newBoard1, depth - 1, false, ref newAIScore, ref newPlayerScore, alpha, beta);

                        // Cập nhật giá trị tốt nhất
                        bestScore = Math.Max(bestScore, score1);
                        alpha = Math.Max(alpha, bestScore);
                        if (beta <= alpha) break; // Cắt tỉa

                        // Mô phỏng nước đi sang trái
                        newAIScore = AIScore;
                        newPlayerScore = playerScore;
                        int[] newBoard2 = (int[])board.Clone();
                        SimulateLeftMove(newBoard2, i, ref newAIScore, ref newPlayerScore, isAI);
                        int score2 = MinimaxAB(newBoard2, depth - 1, false, ref newAIScore, ref newPlayerScore, alpha, beta);
            ...
            else
            {
                ...
                // Xét tất cả các ô của người chơi (ô từ 0 đến 4)
                for (int i = 0; i < 5; i++)
                {
                    if (board[i] > 0)
                    {
                        ...
                        int[] newBoard1 = (int[])board.Clone();
                        SimulateRightMove(newBoard1, i, ref newAIScore, ref newPlayerScore, isAI);
                        int score1 = MinimaxAB(newBoard1, depth - 1, true, ref newAIScore, ref newPlayerScore, alpha, beta);

                        // Cập nhật giá trị tốt nhất
                        bestScore = Math.Min(bestScore, score1);
                        ...
                        int[] newBoard2 = (int[])board.Clone();
                        SimulateLeftMove(newBoard2, i, ref newAIScore, ref newPlayerScore, isAI);
                        int score2 = MinimaxAB(newBoard2, depth - 1, true, ref newAIScore, ref newPlayerScore, alpha, beta);

                        // Cập nhật giá trị tốt nhất
                        bestScore = Math.Min(bestScore, score2);
                        beta = Math.Min(beta, bestScore);
                        if (beta <= alpha) break; // Cắt tỉa
                    }
                }
